...
[
  ...
  if test x"$with_Qt_dir" = x"no" ||
    ...
    dnl Added by gmb 7/23/05
    # set reasonable defaults for all values
    if test x"$with_Qt_dir" = x; then
      with_Qt_dir=$QTDIR
    fi
    if test x"$with_Qt_include_dir" = x; then
      with_Qt_include_dir=$with_Qt_dir/include
    fi
    if test x"$with_Qt_bin_dir" = x; then
      with_Qt_bin_dir=$with_Qt_dir/bin
    fi
    if test x"$with_Qt_lib_dir" = xyes; then
      with_Qt_lib_dir=$with_Qt_dir/lib
    fi
    if test x"$with_Qt_lib" = xyes; then
      with_Qt_lib=$with_Qt_dir/lib
    fi
    dnl end of my addition. -gmb
