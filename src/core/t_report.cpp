...
/* t_report.cpp
   ...
   Program to test the ReadReport class to make sure that it
   works as designed.
...
class TestReadReport : public Tester
{
...
void TestReadReport::constructors(void)
{

	const char fn[] = ".temptest_report";
	fstream fp(fn,ios::out);
	fp << _rep_good_dat;
	fp.close();

	ReadReport r(fn);

	Test( r.beginning()=="10/01/04", __Tester__ );
	Test( r.ending()=="10/31/04", __Tester__ );

	Test( r.more()==true, __Tester__ );
	Test( r.acct()=="Income:Salary", __Tester__ );
	Test( (r.val()-1048.16)<eps, __Tester__ );

	Test( r.more()==true, __Tester__ );
	Test( r.acct()=="Expenses:Tithe", __Tester__);
	Test( (r.val()-105.00)<eps, __Tester__ );

	Test( r.more()==true, __Tester__ );
	Test( r.acct()=="Assets:Current Assets:Savings Account",
	      __Tester__ );
	Test( (r.val()-100.00)<eps, __Tester__ );

	Test( r.more()==true, __Tester__ );
	Test( r.acct()=="Expenses:Grocery", __Tester__ );
	Test( (r.val()-275.41)<eps, __Tester__ );

	Test( r.more()==true, __Tester__ );
	Test( r.acct()=="Expenses:Misc", __Tester__ );
	Test( (r.val()-136.25)<eps, __Tester__ );

	Test( r.more()==true, __Tester__ );
	Test( r.acct()=="Expenses:Rent", __Tester__ );
	Test( (r.val()-249.00)<eps, __Tester__ );

	Test( r.more()==true, __Tester__ );
	Test( r.acct()=="Expenses:Utilities:Electric",
	      __Tester__ );
	Test( (r.val()-74.18)<eps, __Tester__ );

	Test( r.more()==true, __Tester__ );
	Test( r.acct()=="Expenses:Auto:Gas", __Tester__ );
	Test( (r.val()-70.20)<eps, __Tester__ );

	Test( r.more()==false, __Tester__ );
	...
	fp.open(fn,ios::out|ios::binary);
	fp << _rep_bad_dat;
	fp.close();

	try {
		r.init(fn);
		Test( false, __Tester__ );
	} catch ( runtime_error e ) {
		Test( true, __Tester__ );
	}		
...
void TestReadReport::methods(void)
{
	// all was tested in constructors
	return;
}

void TestReadReport::operators(void)
{
	// all was tested in constructors
	return;
}
